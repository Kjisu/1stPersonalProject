...
/////////////////////////////////////////////// 사용자가 작성한 게시글 디비에 저장해주는 메서드
	...
	public void insertBoard(BoardBean bb){
		...
		// 디비에 데이터 넘겨주기 전에 게시글 번호부터 손보자.
			SQL = "select max(num) from camping_deal";
			...
			SQL = "insert into camping_deal (num,writer,pass,subject,content,"
...
//////////////////////////////////////////////////디비에 게시글이 몇개 있는지 세어주는 메서드 (int 값 반환)
	...
	public int getBoardCount(){
		...
		try {
			...
			SQL = "select count(*) from camping_deal";
			pstmt = con.prepareStatement(SQL);
			rs = pstmt.executeQuery();
			...
			System.out.println("dealDAO 글 개수 : "+cnt);
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
			closeDB();
...
//////////////////////////////////////////// 글 불러오기 전 조회수 증가 시키는 메서드
	...
	public void updateReadNumber(int num){
		try {
			...
			SQL = "update camping_deal set readcount=readcount+1 where num=?";
			pstmt = con.prepareStatement(SQL);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
...
//////////////////////////////////////// 디비에서 특정 게시글 꺼내오기
	...
	public BoardBean getBoard(int num){
		...
		try {
			...
			SQL = "select * from camping_deal where num=?";
			pstmt = con.prepareStatement(SQL);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
...
////////////////////////////////////////// 특정 글의 내용 수정한 내용 디비에 넣어주는 메서드
	...
	public int updateContent(BoardBean bb){
		...
		// 글쓴이가 입력한 비밀번호와 디비에 존재하는 글의 비밀번호가 일치하는지 체크
			SQL = "select pass from camping_deal where num=?";
			pstmt = con.prepareStatement(SQL);
			...
			if(rs.next()){
				...
				// 일치하다면 업데이트 시키자.
					SQL = "update camping_deal set subject=?,content=? where num=?";
					pstmt = con.prepareStatement(SQL);
	...
	public int deleteBoard(BoardBean bb){
		...
		// 본인이 작성하 글이 맞는지 확인 
			...
			if(rs.next()){
				if(bb.getPass().equals(rs.getString("pass"))){
				// 본인 글이 맞으니까 삭제 작업시작
					SQL = "delete from camping_deal where num=?";
					pstmt = con.prepareStatement(SQL);
					pstmt.setInt(1, bb.getNum());
			...
			System.out.println("중고게시판dao: deleteBoard()메서드에서 쿼리 구문 실행 실패---이유>>"+e.getMessage());
			e.printStackTrace();
		}finally{
			closeDB();
		}
...
////////////////////////////////////////// db에 답글 넣기
	...
	// re_ref 값이 같은 그룹 중, re_seq 기존의 값보다 큰 값이 있을 때 1증가 시키기 (조건문)
			SQL = "update camping_deal set re_seq=re_seq+1 where re_ref=? and re_seq>?";
			pstmt = con.prepareStatement(SQL);
	...
	// 3. 답글 데이터 db에 넣기
			SQL = "insert into camping_deal values(?,?,?,?,?,?,?,?,?,now(),?,?);";
			pstmt = con.prepareStatement(SQL);
			...
			System.out.println("중고 게시판 dao:db에 답글 저장 완료---");
		} catch (Exception e) {
			System.out.println("중고 게시판 dao: insertReBoard()메서드 중 쿼리 구문 실행 실패 이유>>>"+e.getMessage());
			e.printStackTrace();
		}finally{
			closeDB();
		}
...
//////////////////////////////////// getBoardCount(search)오버로딩 : 사용자가 입력한 검색어에 부합되는 게시글 개수 알려주는 메서드
	
	public int getBoardCount(String search,String type){
...
/////////////////////////////////////// getBoardList() 오버로딩 : 검색어가 있는 경우, 검색어에 부합되는 데이터 출력해주는 메서드
