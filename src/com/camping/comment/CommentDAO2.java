...
///////////////////////////////////////// 	게시글 저장해 주는 메서드
	...
	public void insertComment(CommentBean cb){
		...
		// 댓글 데이터 디비에 저장하기 전에 댓글 번호부터 작업
			SQL = "select max(comment_no) from board_comment2";
			...
			SQL = "insert into board_comment2 values(?,?,?,?,?,?,now(),?)";
			pstmt = con.prepareStatement(SQL);
			pstmt.setInt(1, cno);
...
//////////////////////////////////////////////// 디비에 저장된 댓글 꺼내주는 메서드
	...
	public ArrayList getCommentList(){
		...
		try {
			...
			SQL = "select * from board_comment2"
...
/////////////////////////////////////////// 디비에 저장된 댓글 갯수 세어주는 메서드
	...
	public int getCommentCount(int num){
		...
		try {
			...
			SQL = "select count(*) from board_comment2"
					+ " where num = ?";
			pstmt = con.prepareStatement(SQL);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
...
/////////////////////////////////////////// 댓글 수정하기
	...
	public void updateComment(CommentBean cb){
		...
		try {
			...
			SQL = "update board_comment2"
					+ " set comment_content=?"
					+ " where comment_no=? and num=?";
			pstmt = con.prepareStatement(SQL);
			pstmt.setString(1, cb.getComment_content());
			pstmt.setInt(2, cb.getComment_no());
			...
			System.out.println("게시글 수정 하기 실패 >>"+e.getMessage());
			e.printStackTrace();
		}finally{
			closeDB();
		}
...
////////////////////////////////////////// 댓글 삭제하기
	...
	public void deleteComment(int comment_no, int num){
		
		try {
			con = getCon();
			
			SQL = "delete from board_comment2"
					+ " where comment_no=? and num=?";
			pstmt = con.prepareStatement(SQL);
