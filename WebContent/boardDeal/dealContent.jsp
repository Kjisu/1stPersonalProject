...
<!-- 댓글 보여지는 곳, 입력하는 곳 html -->	
	...
	<table>
		...
		<% 
			if(member_id != null && member_id.equals(cb.getMember_id())){
		%><td>
				<form action="commentUpdate.jsp?#update">
					<input type="submit"  value="수정하기" >
					<input type="hidden" value="<%=cb.getComment_no() %>" name="commentNum">
					<input type="hidden" value="<%=num %>" name="num">
				</form>
				<form action="commentDelete.jsp"> <!-- 댓글 번호, 글 번호 hidden으로 넘겼음 -->
...
<%-- 	<script type="text/javascript">
	...
	// 새창 열어서 댓글 수정 내용 입력하기
 		function reUpdate(){ 
 			window.open("commentUpdate.jsp?num="+<%=num%>+"&comment_no="+<%=cb.getComment_no()%>+"&"+<%= %>);
 			//window.open("commentUpdate.jsp","","width=500, height=300");
 	}
