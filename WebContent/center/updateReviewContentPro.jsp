...
<body>
	...
	<%
		...
		// 주소에 붙어서 get방식으로 넘어온 데이터 저장하기
		String pageNum = request.getParameter("pageNum");
	
		// form태그에 의해 post방식으로 넘어온 데이터 저장하기 (맴버빈 객체에 - 액션태그 이용)
	%>
		<jsp:useBean id="bb" class="com.camping.board.BoardBean"/>
		<jsp:setProperty property="*" name="bb"/>
		<%System.out.println("게시글 수정 pro : 자바빈 객체에 수정된 데이터 잘 담겼낭 ==>"+bb);%>
	<%
		// db에 업데이트 시키기
		ReviewBoardDAO rbdao = new ReviewBoardDAO();
		int result = rbdao.updateReviewContent(bb);
		...
		if(result == 1){
			...
			<script type="text/javascript">
				var result = confirm('수정된 게시글로 이동하시겠습니까?');
				if(result){
					location.href='reviewContent.jsp?num=<%=num%>';
				}else{
