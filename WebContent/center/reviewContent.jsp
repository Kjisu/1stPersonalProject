...
<!-- 이미지를 올린 경우에만 보이도록 제어 -->
	...
	<%} %>
<!-- 업로드한 파일이 있는 경우에만 보이도록 제어 -->
	<% 
		if(bb.getFile() != null){
	%>
		<tr>
			<td>첨부파일</td>
			<td colspan="4"><a href="fileDown.jsp?filename=<%=bb.getFile() %>"><%=bb.getFile() %></a></td>
		</tr>
	<% 
		}
	%>
	...
	%>
		<input type="button" value="수정하기" onclick="location.href='updateReviewContentForm.jsp?pageNum=<%=pageNum %>&num=<%=num %>'">
...
<!-- 댓글 보여지는 곳, 입력하는 곳 html -->	
	...
	<ul>
		...
		%>	
			...
			<%=cb.getDate() %>
			<!-- <input type="button" name="re" value="수정하기" onclick="';">
	...
	<fieldset>
		<form action="commentPro.jsp?num=<%=bb.getNum() %>" method="post" >
			...
			<textarea rows="5" cols="60" name="comment_content">댓글을 남겨보세요</textarea>
	...
	<script type="text/javascript">
		function delQuestion(){
			...
			if(result){
				location.href='deleteReviewBoardForm.jsp?num=<%=num%>&pageNum=<%=pageNum %>';
			}else{
				location.href='reviewContent.jsp';
			}
		}
	</script>
